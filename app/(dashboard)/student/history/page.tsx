            ...
            <div className="grid gap-6">
                {attempts.length === 0 ? (
                    ...
                    attempts.map((attempt) => (
                        <div
                            key={attempt.id}
                            className="bg-white dark:bg-gray-800 rounded-3xl p-6 border-2 border-gray-100 dark:border-gray-700 hover:border-indigo-200 dark:hover:border-indigo-900 transition-all hover:shadow-lg group"
                        >
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                <div className="flex items-start gap-4">
                                    <div className={`
                                        w-16 h-16 rounded-2xl flex items-center justify-center text-2xl font-bold shrink-0
                                        ${(attempt.score || 0) >= 50 ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400'}
                                    `}>
                                        {(attempt.score || 0) >= 50 ? <Trophy className="h-8 w-8" /> : <XCircle className="h-8 w-8" />}
                                    </div>

                                    <div>
                                        <h3 className="font-bold text-xl text-gray-900 dark:text-white mb-1 group-hover:text-indigo-600 transition-colors">
                                            {attempt.exam.title}
                                        </h3>
                                ...
                                <div className="flex items-center gap-8 border-t md:border-t-0 border-gray-100 dark:border-gray-700 pt-4 md:pt-0">
                                    <div className="text-center">
                                        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Score</p>
                                        <p className={`text-3xl font-black ${(attempt.score || 0) >= 50 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                            {attempt.score}
                                        </p>
                                    </div>

                                    {/* Placeholder for future "View Details" feature */}
                                    {/* <button className="bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-600 dark:text-gray-300 px-4 py-2 rounded-xl font-bold text-sm transition-colors">
                                        Details
                                    </button> */}
                                </div>
                            </div>
